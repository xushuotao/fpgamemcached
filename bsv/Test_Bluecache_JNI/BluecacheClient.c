...
int reqCnt = 0;
JNIEXPORT jboolean JNICALL Java_BluecacheClient_sendSet(JNIEnv *env, jobject obj, jbyteArray key, jbyteArray val, jint threadId){
...
JNIEXPORT jbyteArray JNICALL Java_BluecacheClient_sendGet(JNIEnv *env, jobject obj, jbyteArray key, jint threadId){ 
  int keylen = env->GetArrayLength(key);
  /* unsigned char* keybuf = new unsigned char[keylen]; */
  /* env->GetByteArrayRegion(key, 0, keylen, reinterpret_cast<jbyte*>(keybuf)); */
  jbyte *keybuf = env->GetByteArrayElements(key,0);
...
JNIEXPORT jboolean JNICALL Java_BluecacheClient_sendDelete(JNIEnv *env, jobject obj, jbyteArray key, jint threadId){
  int keylen = env->GetArrayLength(key);
  unsigned char* keybuf = new unsigned char[keylen];
  env->GetByteArrayRegion(key, 0, keylen, reinterpret_cast<jbyte*>(keybuf));
...
JNIEXPORT jdouble JNICALL Java_BluecacheClient_getGetAvgLatency(JNIEnv *env, jobject obj){
